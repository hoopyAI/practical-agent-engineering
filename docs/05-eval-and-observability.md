...
| **Trial** | A single attempt on a given task (model output is stochastic, so multiple trials are needed) |
...
2. **Extract tasks from real failures,** not made up scenarios. Use things that actually caused problems.
...
**Tag every token with metadata:** agent ID, task type, conversation thread.
...
| Resource | Description |
|----------|-------------|
| [Anthropic: Demystifying Evals](https://www.anthropic.com/engineering/demystifying-evals-for-ai-agents) | **Start here.** Evaluation methodology primer. |
