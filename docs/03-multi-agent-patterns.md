...
When a single Agent handles everything, the prompt grows increasingly complex and capabilities become diluted. Multi-Agent architectures split tasks across specialized Agents — like going from a monolithic application to microservices.
...
**When to use:**
- Tasks that can be clearly split into independent subtasks
...
Agents pass control to each other via **explicit handoff functions**. Not a fixed sequence — instead, the next handler is decided dynamically based on context.
...
- But Swarm's handoff design was influential — the Agents SDK inherits it
...
This is essentially a **Subagent + Routing** combination, using the Gateway for routing and isolation.
...
| [Agent Swarms: Multi-Agent Networks Guide](https://teammates.ai/agent-swarms) | Swarm pattern landscape |
